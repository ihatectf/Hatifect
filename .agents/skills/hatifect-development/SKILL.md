...
2. Select applicable routes from `routing.json`. Read their scoped instructions even from a root-cwd task. Repository-owned `hatifect-*` route skills are portable workflows in the adjacent skills directory; other names are recommendations to locate in the current catalog and may be unavailable. Choose the smallest relevant set, read each selected skill fully and announce it. Do not load every route or equate discovery with use.
