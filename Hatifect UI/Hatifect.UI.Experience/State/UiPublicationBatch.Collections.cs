...
public sealed partial class UiPublicationBatch
{
    ...
    public UiPublicationBatch Apply<T>(UiPublishedCollection<T> source, UiCollectionChange<T> change)
    {
        ...
        if (_changes.ContainsKey(source.SourceId) || !_explicitCollectionChanges.Add(source.SourceId))
        { Reject(source, "UIP015", "Use one explicit collection change per source and publication batch."); return this; }
        ...
        if (change.Version <= previous.Version)
        {
            try
            {
                if (previous.History.Any(applied => applied.Equivalent(change))) return this;
