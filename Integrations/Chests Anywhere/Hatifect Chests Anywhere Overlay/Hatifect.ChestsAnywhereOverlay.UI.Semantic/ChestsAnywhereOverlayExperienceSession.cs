...
internal sealed class ChestsAnywhereNavigatorExperienceSession : IDisposable
{
    ...
    private readonly UiState<ChestsAnywhereNavigatorMode> _mode;
    private readonly UiState<string> _selectedCategory;
    private readonly UiState<string> _status;
    private readonly UiState<ChestsAnywhereStorageHandoff?> _handoff = new(null);
    private readonly UiSelectableCollectionState<ChestsAnywhereNavigatorCategory> _categories;
    private readonly UiSelectableCollectionState<ChestsAnywhereNavigatorStorage> _storages;
    private NavigatorProjection _projection;
        ...
        Func<string, string, string>? translate = null)
    {
        ...
        _projection = Project(port.Capture());

        _mode = new UiState<ChestsAnywhereNavigatorMode>(_projection.Mode);
        _selectedCategory = new UiState<string>(_projection.SelectedCategoryKey);
        _status = new UiState<string>(_projection.StatusText);
        _categories = new UiSelectableCollectionState<ChestsAnywhereNavigatorCategory>(
            _projection.Categories,
            category => category.Id,
            category => category.Label,
            CategoryId(_projection.SelectedCategoryKey));
        _storages = new UiSelectableCollectionState<ChestsAnywhereNavigatorStorage>(
            VisibleStorages(_projection, _projection.Mode, _projection.SelectedCategoryKey),
            storage => storage.Id,
    ...
    public UiState<ChestsAnywhereNavigatorMode> Mode => _mode;
    public UiState<string> SelectedCategory => _selectedCategory;
    public UiState<string> Status => _status;
    public UiSelectableCollectionState<ChestsAnywhereNavigatorCategory> Categories => _categories;
    public UiSelectableCollectionState<ChestsAnywhereNavigatorStorage> Storages => _storages;
    public UiState<ChestsAnywhereStorageHandoff?> Handoff => _handoff;
    ...
    public void SelectCategory(string categoryKey)
    {
        ...
        if (!_projection.Categories.Any(category => string.Equals(category.Key, categoryKey, StringComparison.Ordinal)))
    ...
    public void OpenSelectedStorage()
    {
        ...
        _handoff.Value = result.Succeeded
    ...
    private bool Available => !_disposed && !_completed;
    ...
    {
        if (_synchronizing || !Available || _categories.SelectedItemId is not { } id) return;
        ChestsAnywhereNavigatorCategory? category = _categories.Value.FirstOrDefault(value => value.Id == id);
        if (category != null && !string.Equals(category.Key, _selectedCategory.Value, StringComparison.Ordinal))
    ...
    private void ApplyProjection(NavigatorProjection next, bool preserveView)
    {
        ...
        try
        {
            ...
            _status.Value = next.StatusText;
            _categories.Replace(next.Categories);
            if (CategoryId(nextCategory) is { } categoryId)
                _categories.TrySelect(categoryId);
            ReplaceVisibleStorages(next, nextMode, nextCategory);
        }
        finally
        {
    ...
    private void ReplaceVisibleStorages(
        ...
        string categoryKey)
    {
        ...
        _storages.Replace(visible);
        UiSymbolId? preferred = prior;
        if (preferred is { } selected && visible.Any(storage => storage.Id == selected))
            _storages.TrySelect(selected);
        else if (_storages.SelectedItemId == null && visible.Count > 0)
            _storages.TrySelect(visible[0].Id);
    }
