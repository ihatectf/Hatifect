...
A complete response carries snapshot identity, `diagnostics`, `decisions`, and a nullable `plan`. `status` is `planned`, `planning-rejected` or `compilation-invalid`. Only `planned` contains an ordered plan with pattern, host/profile and element regions/presentations. Decisions preserve code, element, message, candidate and nullable source provenance (`sourceName`, `start`, `length`, `line`, `column`). Rejected planning and invalid compilation return no partial plan. Stale metadata, invalid environment and response overflow return protocol errors. This operation never activates runtime surfaces. See [«Обмен данными с редактором» acceptance](editor-protocol-acceptance.md#обмен-данными-с-редактором-подэтап-d--actual-server-planner-trace) for behavioral evidence.
