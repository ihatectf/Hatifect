...
| Show empty and faulted states | `ParcelExperience.IsActive` now retains current-session missing/faulted views. `ParcelTextValue` distinguishes missing/faulted/closed reasons and retains pause/recovery availability. The surface refreshes these states with all unavailable actions blocked; Closed or explicit disposal retires it; the application now publishes Closed even when disposed after Faulted. | Native `show` still rejects a missing parcel before creating the experience. Finish that opening path and prove actual empty/faulted rendering and game lifecycle; the host-free tests alone do not close native acceptance. |
