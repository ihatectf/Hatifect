# «Базовый контракт интерфейса» / «Совместимость публичного API» — минимальный semantic-v2

Дата: 2026-09-06. Решение принято в пределах явно утверждённой roadmap. База: «Аудит исходной базы», commit `de766c5`, код `55b8360`. Этот документ задаёт контракт для «Типы и связи данных интерфейса»–«Адаптация интерфейса к окружению»/«Данные и команды Flowline»; перечисленные новые типы становятся реализованными только вместе с кодом и acceptance своего среза.

## «Совместимость публичного API»: один graph и существующий runtime
...
Compatibility path ограничен одним понижением старых builder overloads в тот же graph. Старые overloads и `IUiSemanticSurfaceApi` v1 сохраняются как поддерживаемая совместимая API; отдельный дублирующий compatibility runtime не вводится. Legacy overload без nominal type/nullability создаёт независимый opaque node с прежними capabilities и source. Это постоянное ограниченное правило совместимости, в том числе для внешнего `.Inspect("Parcel", new UiConstantSource<MyRecord>(...))`; «Интеграция Chests Anywhere» его не удаляет. Opaque node не может участвовать в typed relations без явного descriptor, не получает выдуманную nullability и сохраняет существующие runtime проверки формы source. Strict pre-activation graph validation применяется к новым typed declarations и relations. Временной является только адаптация repository Flow/CA declarations: после миграции в «Простое создание диалогов и экранов»/«Интеграция Chests Anywhere» удаляется их opaque authoring path и соответствующие private adapters; сам совместимый public overload остаётся. Новые Flow/CA модели сразу используют explicit typed declarations. Неоднозначность нового typed graph всегда даёт diagnostic, а не расширение legacy эвристики.
...
Для «Типы и связи данных интерфейса» резервируется следующий UI producer version `1.0.0-alpha.32`: `.30` принадлежит Flow foundation `cdf9d2e`, `.31` объединяет его с ранее проверенными SDK/editor срезами и сохраняет оба API форм. Новая версия исключает использование прежнего `.30` из общего package cache. До package verification «Типы и связи данных интерфейса» синхронно обновляются все восемь producer packages, package authority/props, UI manifest/release follower versions и минимальная UI dependency всех affected consumer. Это локальная версия кандидата, не публикация. Перед интеграцией Flow сохраняется более новая общая authority и повторяются C/G/P на объединённом коде. В runtime-архиве единственный поставщик UI DLL — UI module; Flow semantic DLL добавляется в «Просмотр состояния Flowline».

## Identity, alias, label и типы («Типы и связи данных интерфейса»)
...
Пример CA Storages: slots `.../input/mode` принимают required nominal `StorageMode`, `.../input/category` — optional nominal `CategoryId`; соответствующие Filter relations связывают Mode и SelectedCategory с разными slot IDs одного target. Flow parcels имеет `.../input/query` required string. Slot metadata и TargetInput сохраняются в graph → binding context → schema-v2 export/import без вывода типов из labels. Acceptance «Типы и связи данных интерфейса» включает валидный round trip двух разных filter inputs и отрицательный import/bind после подмены slot/type/required relation. Binder возвращает отдельные адресные diagnostics с node/relation/input ID и endpoints; не бросает общую ошибку после регистрации host. Build с ошибкой не запечатывает builder, не подписывает источники и не активирует session.
...
Обязательные «Типы и связи данных интерфейса» regression paths: explicit element и role ID → смена alias → Export/Import → Compile → тот же ID в IR; прежний canonical v1 byte round trip; прежние v1 conflicts/duplicates; unknown schema; неверный/cross-owner ID; valid Flow и package-only CA graph fixtures; все negative relations до activation. Минимальная v2 wire часть входит в «Типы и связи данных интерфейса», иначе новый публичный Export был бы сломан до «Обмен данными с редактором». Полный planner trace/editor feedback остаётся «Обмен данными с редактором».

### Реализация «Типы и связи данных интерфейса»: alpha.32
...
Существующий `UiFormState` экспортирует `IReadOnlyList<UiSemanticFormField>`; `UiSourceTypes.Form(schema)` проверяет именно этот контракт. `UiValidationResult` хранит immutable schema/messages. `Submission.Mapping` явно описывает output consumer adapter; «Типы и связи данных интерфейса» не исполняет этот adapter и не добавляет вымышленный `UiFormSnapshot` или async dispatch. Их дальнейший контракт относится к «Согласованное обновление данных»/«Действия и их завершение». Новые typed enum/record filters являются auxiliary sources до появления подходящего editable presentation; предъявленный Filter должен иметь поддержанный string source.

Flow Network использует независимые ID/alias/локализованные labels, typed immutable parcel collection, owner selection, nullable details payload и query/filter slots. Добавление Browse для Query/Filter включает действующую framework policy: History выбирает Gallery в Wide/Medium, List в Compact/Controller, generated Network pattern становится MasterDetail. CA объявляет Mode и Category как разные required inputs Storages, две owner selection relations и Open/Favorite ActionTarget. Пассивное старое CA Mode presentation пока использует совместимый opaque overload; private bridge удаляется в «Простое создание диалогов и экранов»/«Интеграция Chests Anywhere» после появления typed choice authoring. CA перешёл на atomic publication в «Согласованное обновление данных» (подэтап a); Parcel и Network используют ту же boundary в «Согласованное обновление данных» (подэтап b).
...
Graph binder использует индексы и итеративный DAG pass; shared descriptor branches не обходятся экспоненциально при validation/equality/hash. Предельная глубина descriptor — 16 edges, wire expansion — 65 536 ненулевых entries, JSON depth — 32; превышение даёт явную ошибку. Tooling считает все nodes/inputs/relations/roles в пределах 4 096 symbols/context и 16 384/session. На draw/layout/dispatch новые graph обходы не добавлены. Результаты проверок и ограничения фиксируются отдельно в [прежний журнал проверок (история Git)](accepted-foundations.md).

## Согласованное состояние («Согласованное обновление данных»)
...
### Реализованная часть «Согласованное обновление данных» (подэтап a)
...
CA mapping «Типы и связи данных интерфейса»/«Согласованное обновление данных»: Mode/Category filter → Storages; Categories → SelectedCategory; Storages → SelectedStorage; Open/Favorite action → SelectedStorage. IDs берутся из существующих provider keys, не labels. Candidate сначала нормализуется и валидируется, потом публикуется целиком. Сохраняются native handoff, capabilities, close/reopen и отсутствие private runtime доступа. «Типы и связи данных интерфейса» включает graph fixture и собираемость exact-package consumer; «Интеграция Chests Anywhere» завершает продуктовую миграцию/runtime matrix.

## Следующие обязательные срезы и acceptance

1. «Типы и связи данных интерфейса»: graph/types/identity/binding/wire целиком, Flow immutable read-model fixture и CA fixture; негативные отношения до activation; C/U/P. До DONE нельзя оставить alias ID loss в Export или объявить только data classes готовым graph.
2. «Согласованное обновление данных» и «Адаптация интерфейса к окружению»: publication/deltas и environment/coverage planner независимо используют «Типы и связи данных интерфейса». «Адаптация интерфейса к окружению» facets: viewport, scale, pointer/keyboard/controller, locale, theme, reduced-motion/high-contrast; determinism и bounded rejected-alternative trace обязательны. Unsupported combination даёт адресную diagnostic/fallback, не исчезновение поля/action.
3. «Данные и команды Flowline»: интеграция готового application candidate с проверкой всех вышеописанных инвариантов и F gate. После «Согласованное обновление данных»/«Адаптация интерфейса к окружению»/«Данные и команды Flowline» — «Просмотр состояния Flowline» с release inventory/host/runtime, затем следующий dependency-ready ID по roadmap.

«Базовый контракт интерфейса» меняет спецификацию, не C# API/persistence/host. Его C gate проверяет существующую базу; U/P/G/RUNTIME/VISUAL/PERF не объявляются выполненными за будущий код. Новые numeric limits publication относятся к bounded contract «Согласованное обновление данных» и должны быть проверены тестами; существующие performance acceptance JSON не меняются. Полная roadmap, включая выбранный editor, Exact, component catalog, extension API и runtime/manual acceptance «Приёмка полного выпуска», сохраняется без сокращения.


«Действия и их завершение» (подэтап b), alpha.40: Runtime.Update подготавливает layout, interaction, render frame и accessibility до принятия сцены. При отказе callback остаются прежние согласованные snapshots и тот же объект input session; повторный вход и изменение input во время подготовки запрещены до эффектов. Отклонённая перестановка или удаление коллекции не становятся исходным порядком для следующего scroll-anchor transition. В штатном Update не копируются measurement caches или adaptive height index; после неуспешной подготовки cache может быть холодным. Это внутренняя граница принятия сцены. Terminal validation/retirement, typed binding и owning Update pump продолжают «Действия и их завершение».

«Действия и их завершение» (подэтап b), alpha.41: private Runtime retirement блокирует будущие host/input mutations и принятие подготовленной сцены после закрытия. Portal закрывает потомков синхронно и проверяет точного владельца/его принятую версию перед регистрацией candidate. Terminal validation не может заменить вложенный успешно принятый frame; legacy host input повторно проверяет владельца после availability до Execute. Публичный direct TryExecute остаётся совместимым. Это предпосылка per-host binding; полноценные generations/reload и transaction для всех input/scroll/draw callbacks относятся к последующим срезам.


«Действия и их завершение» (подэтап b), alpha.42: `UiAction<TRequest,TResult>.Bind(capture, completed, owner)` создаёт immutable `UiActionDefinition` для существующего Experience action group/contribution. Consumer сохраняет точный экземпляр definition при обычной композиции и предоставляет immutable request snapshot. Request factory вызывается один раз после первоначального admission; версия принятого кадра повторно проверяется после availability, capture и RestartLatest cancellation callback. При отказе повторной проверки эффект не начинается. Типы запроса/result сохраняются через внутренний generic factory без reflection/dynamic и зависимости Experience от Runtime.

Host владеет текущей картой максимум4096 bindings. Prepare сохраняет execution у того же definition, создаёт новые отдельно и не отменяет принятые операции при неудаче. Commit переключает карту вместе со сценой; membership fence немедленно запрещает все удалённые bindings до их cancellation callbacks. Одинаковые модели в двух hosts имеют независимые execution state/очереди. Optional `UiPublication` отзывает свои bindings до повторного входа в эффекты; отписка происходит при discard/replacement/dispose. Owning root Pump доставляет completion и обновляет захваченную доступность; Render, диагностика и композиция не являются pump. Production Stardew Update, explicit Open/Hide/reload/save-switch generations и сквозной typed consumer/RUNTIME ещё не подключены, поэтому полный «Действия и их завершение» остаётся IN_PROGRESS.

Публичный direct `TryExecute()` legacy остаётся синхронным, включая исходные exceptions. В host его unit adapter наблюдает failures в общем action result и проверяет availability один раз при admission. Legacy CanExecute по-прежнему валидирует candidate в focus/accessibility preparation; значения подготавливаются отдельно от accepted execution. Typed definition без host сообщает `CanExecute=false`, а direct TryExecute выдаёт явный отказ. Framework input/render/accessibility используют captured host eligibility; диагностический snapshot содержит action state, queue count, reason/outcome и типы ошибок без request/result payload или вызова `Exception.ToString()`.

Disabled button использует общий opacity token. Typed button Hover/Focused/Pressed/Disabled recipes допускают только render effects и проверяются до принятия сцены. Legacy geometry recipes продолжают применяться через обычную композицию и layout; между композициями host обновляет только render properties относительно принятой геометрии. Scene-owned caches ограничены16 combinations; legacy дополнительно хранит максимум16 render projections. Смена только Disabled recipe у выполняющегося действия обновляет frame при сохранении layout.
