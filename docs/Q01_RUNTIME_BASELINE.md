...
Next: correct the standalone HUD-overlay fixture and inspect its rendered evidence, then address the separate viewport/composition and visual-matrix observations. The capture correction and its fresh C/G/P/runtime evidence are recorded separately in [Q01_CAPTURE_FIX.md](Q01_CAPTURE_FIX.md). Full Q01 remains IN_PROGRESS while these gaps are unresolved. The baseline above remains attached to `7a9e590` when newer Flow/UI changes are integrated.
