# Q01: runtime baseline from a Codex terminal

Status: **IN_PROGRESS**. This checkpoint owns the fresh combined runtime baseline and its evidence. The UI task continues U02 and the Flow task continues F17; their implementation and historical runtime results are not relabelled by this report.

## Candidate and environment

- Candidate: `7a9e590080b05fbcb7df3f473ec67a10050d4610`, published in `develop`; UI package authority `1.0.0-alpha.35`.
- Worktree: `${HOME}/Developer/Worktrees/Codex/345f/Hatifect`.
...
The root instructions authorize required isolated runtime checks without repeated conversational approval. The project rule permits the exact canonical `rtk proxy ./tools/hatifect-runtime-executor serve` prefix when effective policy allows it. Managed/OS restrictions remain in force. Invocation and process ownership are documented in [RUNTIME.md](RUNTIME.md).

The actual Codex PTY launch reached Ready for this exact worktree and `doctor` returned PASS. The first bounded startup/shutdown probe is retained in `artifacts/runtime-executor-terminal/`. The alpha.35 executor completed this series and was stopped through its owning PTY before integrating the next source revision. Ready is not game acceptance.
...
`rtk proxy ./tools/hatifect-agent-check --host` passed after the executor configuration was integrated: project config and project skill loaded, 217 skills discovered. The command proves configuration discovery, not model compliance or absence of stricter effective permission rules.
...
- The aggregate screenshot contains the game room while diagnostics still report `terminalOpen: true`. The capture reads `GraphicsDevice.GetBackBufferData` during `Rendered`; [SMAPI 4.5.2 raises that event with the current render target bound](https://github.com/Pathoschild/SMAPI/blob/4.5.2/src/SMAPI/Framework/SCore.cs#L1183-L1238). The capture source therefore needs a bounded correction before that screenshot can support visual acceptance.
...
Next: correct screenshot capture as a separate bounded fix and rerun the affected game scenarios. Full Q01 remains IN_PROGRESS while these visual evidence gaps are unresolved. The baseline above remains attached to `7a9e590` when newer Flow/UI changes are integrated.
