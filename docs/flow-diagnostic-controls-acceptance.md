# Приёмка управления диагностической перевозкой
...
Статус: **IN_PROGRESS**. «Управление диагностической перевозкой» (подэтап a) опубликован в [`7d62ab5`](https://github.com/ihatectf/Hatifect/commit/7d62ab509ac6987df76938c4f34612669b3de16f), база — `203788f71da542470450e1b6aebf69f4a17135fb`, ветка `feature/flow-f13-actions`. Эта запись описывает проверенный объём consumer implementation и оставшуюся приёмку; полный «Управление диагностической перевозкой» ещё не завершён.

## Зависимости и границы

«Просмотр состояния Flowline» принят на общем source `08ecdb9` ([приёмка](flow-live-screen-acceptance.md)). «Действия и их завершение» typed action contract и его reload/save-switch retirement проверены на source `9b6028c0561c5e1a8225bcdec7f1bcce64d81ead`: request `02565027-d1ae-4657-9693-c1872318761c` (18 save-switch checks) и `9fb55869-dbe5-4a63-a56b-c95015a89558` (18 reload checks), fingerprint `5fc2c2111b8357967c3c2be5343b12feaa0cad4279033d5fe58a13eaa2471480`. Локальный аудит `artifacts/integration/u03-dependency-native-audit.json` сверяет request/fingerprint и 43 raw hashes. Это подтверждает входной контракт; полная общая alpha47 acceptance и публикация «Действия и их завершение» не объявляются завершёнными.

Owning implementation «Управление диагностической перевозкой» (подэтап a) — Flow.UI.Semantic. Core, persistence и публичные UI API не изменены. Reserve (UI Dispatch), Cancel и RetryDelivery используют существующий доменный контракт через typed host. ReconcileTransfer и ReturnToSource сохраняют прежнюю реализацию; новые операции и гарантии возврата груза не вводятся.
...
- Existing Parcel lifecycle/publication/locale tests вызывают действия через настоящий owning host. Native «Просмотр состояния Flowline» retired-action probe тоже использует action automation вместо недопустимого direct invocation typed action.
...
| Независимый source/test review | scoped source и actual TRX | Доказанных дефектов нет; native/create/G не входили в review |
...
4. Выполнить второй проход по итоговому diff, записать commit/publish evidence и только после полной матрицы пересмотреть статус «Управление диагностической перевозкой».
...
## «Управление диагностической перевозкой» (подэтап b): подготовка fake create
...
Следующий шаг — физический ввод и оставшаяся «Управление диагностической перевозкой» матрица, затем обычный игровой путь «Настройка станций и маршрутов»/«Отправка груза и работа с ошибками». Устанавливаемый архив с полной «Приёмка первой альфы» acceptance ещё не подготовлен. Source866 на момент этого отчёта локальный; публикация и CI для него пока не подтверждены.
