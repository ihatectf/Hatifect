...
Recorded source: `53a388e2115678f9374041812b67095f7402fdcc` on the then-local branch
`feature/u06-tooltip-delay`. At the time of this report, the slice had not been pushed.
...
the full EN/RU/scale/theme matrix and Flow/CA adoption remain open, so «Общие компоненты, темы и подсказки» stays `IN_PROGRESS`.
