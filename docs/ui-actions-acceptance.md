...
Status: **DONE** for the original «Действия и их завершение» criteria in `ROADMAP.md`, accepted on source `9b6028c0561c5e1a8225bcdec7f1bcce64d81ead`. This is a verified implementation checkpoint, not publication of the common candidate or completion of «Проверка текущей сборки в игре»/«Обновление интерфейса без лишних пересчётов».
...
Artifact root: `artifacts/alpha47-u03-final-integration/` in the integration worktree. `common-source-9b6028c.json` binds 637 tracked files to the accepted source. Historical owner and earlier common results retain their original identities in [Сообщения о выполнении действий](ui-action-messages.md).
...
Normalized CA Tab/Enter does not prove physical OS/SMAPI keyboard/controller delivery. «Проверка текущей сборки в игре» Backspace remains open. These message captures do not claim complete product localization or intermediate visual states of every CA action. Full incremental runtime behavior/performance belongs to «Обновление интерфейса без лишних пересчётов»; the broader common acceptance matrix and publication remain separate.

After these accepted runs, aggregate `bcbf231d-4f30-46d6-8767-eb05ab1ab0ea` reported behavioral PASS, but its preparation changed all eight UI DLL hashes without changing HEAD or the 637 source hashes. New DLLs lost the SHA suffix in InformationalVersion and changed their PDB source path. Their game/producer bytes match each other but differ from P `c5vynfny`; the aggregate therefore does **not** receive common package-identity PASS. `all-package-identity-failure.json` retains the discrepancy. The accepted «Действия и их завершение» snapshots above are not overwritten or relabeled. The build-metadata cause and a new consistent package/runtime candidate must be resolved before common publication.

«Действия и их завершение» now satisfies the dependency for «Управление диагностической перевозкой». FLOWLINE can continue original fake-session create/dispatch/cancel/retry/diagnostic acceptance; UI continues «Обновление интерфейса без лишних пересчётов». The integration owner resolves the newly observed build-identity issue and completes the remaining common gates before publishing.
