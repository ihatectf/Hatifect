...
Roadmap утверждена пользователем для последовательной реализации до всех acceptance criteria. Переходы между срезами уже разрешены. На 2026-09-06 `B01` — **IN_PROGRESS**: [аудит текущей базы](ROADMAP_BASELINE.md) сверяет сохранённые UI SDK/editor срезы `55ccc3b`/`55b8360` с полным объёмом roadmap. Остальные ID пока **PLANNED**; наличие частичной основы не означает DONE. Статус DONE появляется только со ссылкой на итоговый commit/PR и выполненную проверку. Календарные обещания и количество PR пока не установлены.
