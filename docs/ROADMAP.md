...
U03 actual save-switch: **IN_PROGRESS**, отдельный owner checkpoint без version bump. Новый native сценарий прошёл 18 проверок: четыре настоящих title transitions, пять загрузок двух owned worlds, четыре public owners, старые completions без callbacks и свежая доставка на owning thread. C — PASS1 528 .NET +372 Python; scoped Stardew — PASS50. [Реализация, evidence и ограничения](U03_SAVE_SWITCH_ACCEPTANCE.md). Immutable source, полный G и внешний P фиксируются следующим шагом; полный U03 ещё требует typed consumer и сообщений.
