...
Общая alpha.47 GQ — **IN_PROGRESS**: frozen Flow text/Parcel `485c815` принят как `e999f44`, UI save-switch `889f9b1` как `c269a50`; owner evidence проверены независимо. Все 17 version authorities принадлежат GQ и переведены на alpha.47. [Состав, контрактные изменения и план общих проверок](U03_U04_F12_ALPHA47_INTEGRATION.md). Новые общие gates пока NOT_RUN, первым выполняется C; полный U03/U04/F12 остаётся открытым. Следующая работа владельцев — actual Flow UI lifecycle и поддерживающее наблюдение принятой поверхности.
