...
Roadmap утверждена пользователем для последовательной реализации до всех acceptance criteria. Переходы между срезами уже разрешены. На 2026-09-06 `B01` — **DONE**, commit `de766c5`: [аудит текущей базы](ROADMAP_BASELINE.md), C gate `artifacts/validation/run-u7blhl0v/summary.json` PASS (947 .NET + 296 Python). `B02` — **IN_PROGRESS**: [минимальный semantic-v2 / D01](SEMANTIC_V2.md). Остальные ID пока **PLANNED**; наличие частичной основы не означает DONE. Статус DONE появляется только со ссылкой на итоговый commit/PR и выполненную проверку. Календарные обещания и количество PR пока не установлены.
