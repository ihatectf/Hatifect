...
Все работы в каталоге ниже имеют статус **PLANNED**. `B01` — ближайшая работа; её первичная инвентаризация начата этим документом. Статус DONE появляется только со ссылкой на итоговый commit/PR и выполненную проверку. Очерёдность — инженерная рекомендация, которую следует уточнять после первых сквозных срезов. Календарные обещания и количество PR пока не установлены.
