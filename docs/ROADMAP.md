...
Исходный каталог был составлен со статусами **PLANNED**. Текущее состояние, доказательства и следующий готовый срез ведутся в [ROADMAP-STATUS.md](ROADMAP-STATUS.md); общий D01 и UI implementation ведутся в отдельном UI worktree с одним владельцем контракта. Статус DONE появляется только со ссылкой на итоговый commit/PR и выполненную проверку. Очерёдность уточняется по зависимостям и фактическому коду. Календарные обещания и количество PR не установлены.
...
Текущие исходники, [архитектура](../ARCHITECTURE.md), [руководство разработки](DEVELOPMENT.md) и исполняемые проверки имеют приоритет над историческими планами.
