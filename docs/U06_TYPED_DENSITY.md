...
- Exact canonical C `run-8lewjf7s`: **PASS 1,908 .NET + 503 Python**, zero failed/skipped — Flow803;
  DevTools10, Planning142, Runtime678, Semantics67, Tooling.Server16, Tooling192; agent setup,
...
- Portable `hatifect-agent-check`: **PASS**. Route host-аудит остановился до audit на локальной Codex
  CLI startup/configuration и не используется как source evidence.
