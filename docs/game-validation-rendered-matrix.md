# Проверка изображения при разных языках и масштабах
...
Both fresh fake Flow runs use `sha256-flow-runtime-v1`, fingerprint `4540912142242796c446029ca28537340391fda9d38cf04ed4701d89998dfb88`. That algorithm covers its own inventory and is not interchangeable with the UI fingerprint. These are current fake-provider lifecycle/isolation results, not a relabelling of the Flow owner's prior real-chest «Ресурсные ограничения Flowline» resource measurements. All five final requests identify source commit `b3265fa`; only documentation changed after their tested compiled inputs were frozen.

All launches use the canonical worktree executor and isolated test root. Build/check/prepare commands use the documented command-local `DOTNET_gcConcurrent=0`; the executor and game run with ordinary GC. Normal Mods, real saves and golden assets are unchanged.

## Remaining «Проверка текущей сборки в игре» scope

Native/physical input is not proved by these programmatic transport checks or screenshots. The prior macOS borderless clipping limitation remains documented; the rendered matrix uses isolated windowed 1280×720. Product-specific empty/loading/error/action states and broader locale/layout coverage belong to their respective acceptance slices. Next ready «Проверка текущей сборки в игре» work is separate native-input observation with explicit distinction between OS-injected events and physical hardware input.
...
The alpha.40 finalizer correction requires current request-owned published diagnostics with no terminal failure, complete matrix/restoration and consistent per-state observations/PNG paths before returning PASS. It preserves the controller's first check verdicts and reports final evidence failure separately. Temporary-file reproduction, focused tests and final static/runtime results are tracked in [the integration report](game-validation-rendered-matrix.md).

The correction is **DONE**, integrated in [`9756790`](https://github.com/ihatectf/Hatifect/commit/9756790604bfc85039d05f86903e781d85661f6a). Fresh alpha.40 matrix `a88ff5f5-1dbd-4261-9f73-67e60538890b` passed all eight checks; all eight completed-frame images were inspected. Late-only fault `12077b0c-9b88-4a3a-8cfa-12072851bc8b` retained eight PASS host verdicts and sixteen matrix PNGs, then correctly produced overall **FAIL / HARNESS-RUNTIME-DIAGNOSTICS**. Its temporary diagnostic retains the original final-write `IOException`; process exit is 0 with no teardown errors. Aggregate `66428992-7d9f-4517-b410-77ed3fe2d451` passed 28 assertions, restoration and 620 performance frames. All three requests identify exact source `9756790` and UI fingerprint `3904aef41f4edc2e18fc834b39993a23bda01a5feea2f05b4775da3ae7b58320`; their artifacts are in `/private/tmp/hatifect-ui-next-ten-slices/artifacts/runtime/`. C/G/P passed on the same implementation. These results close this finalization defect; full «Проверка текущей сборки в игре» and «Действия и их завершение» retain their remaining scope.
