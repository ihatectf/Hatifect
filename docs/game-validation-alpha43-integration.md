# Совместная проверка в игре: alpha.43
...
Ограниченная общая приёмка alpha.43 — **PASS**. P2 повторного захвата keyboard lease после неуспешного Hide подтверждён нативным RED→GREEN и исправлен. Итоговый source commit — [`6c30713`](https://github.com/ihatectf/Hatifect/commit/6c307137287a9a5634c4383d9773cc873bf1a068). Полные «Действия и их завершение» и «Проверка текущей сборки в игре» остаются **IN_PROGRESS**.
...
Эти результаты относятся к owner candidate и fingerprint `0502d9a683a9fabb4d1e7f486a2deb78a6bc43e2a3084e5df802da892d1fd8ad`. Они не объявляются результатами новых DLL общей интеграции. Предыдущие V1 и неуспешные попытки сохранены в [журнале «Действия и их завершение»](ui-actions-acceptance.md).

## Собственная приёмка GQ

Проверки общей интеграции выполняются в `${HOME}/Developer/Worktrees/hatifect/345f/Hatifect`. Логи и отдельные byte/TRX audits сохраняются в `artifacts/alpha43-integration-preflight/`.
