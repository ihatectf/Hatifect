...
internal static class DurableCoreCodec
{
    ...
    public static DurableCoreImage Decode(ReadOnlySpan<byte> bytes)
    {
        ...
        if (bytes[36] == 1)
        {
            intent = new TransferKey(new Guid(bytes.Slice(37, 16)),
                BinaryPrimitives.ReadInt32LittleEndian(bytes[53..]), BinaryPrimitives.ReadInt32LittleEndian(bytes[57..]));
        }
        else if (bytes[36] == 0 && bytes.Slice(37, 24).SequenceEqual(new byte[24]))
        { intent = null; }
        else { throw new InvalidDataException("Durable Core intent discriminator or reserved bytes are invalid."); }
        ...
        if (version >= 2)
        {
            ...
            { admission = new AdmissionIntent(new Guid(bytes.Slice(106, 16)), bytes[122] == 1, bytes[123] == 1); }
            else if (bytes[105] != 0 || !bytes.Slice(106, 18).SequenceEqual(new byte[18]))
            { throw new InvalidDataException("Durable Core admission discriminator or reserved bytes are invalid."); }
        }
        StationRegistrationIntent? registration = null;
        if (version >= 3)
        {
            if (bytes[124] == 1 && bytes[149] <= 1 && bytes[150] <= 1)
            {
                registration = new StationRegistrationIntent(new Guid(bytes.Slice(125, 16)),
                    BinaryPrimitives.ReadInt32LittleEndian(bytes[141..]),
                    BinaryPrimitives.ReadInt32LittleEndian(bytes[145..]), bytes[149] == 1, bytes[150] == 1);
            }
            else if (bytes[124] != 0 || !bytes.Slice(125, 26).SequenceEqual(new byte[26]))
            { throw new InvalidDataException("Durable Core registration discriminator or reserved bytes are invalid."); }
        }
        CargoProvisionIntent? provision = null;
        if (version >= 4)
        {
            if (bytes[151] == 1 && provisionLength > ProvisionPrefixLength)
            {
                ...
                provision = new CargoProvisionIntent(new Guid(bytes.Slice(headerLength, 16)),
