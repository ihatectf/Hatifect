...
public sealed class ResourceRetentionTests
{
    ...
    [InlineData(128)]
    [InlineData(256)]
    public void ProductionBounds_RetainEveryReceiptAndRejectAttemptSeventeenWithoutEffects(int count)
    {
        ...
        int effects = 0;
        PortTransfer? firstDelivery = null;
        var source = new SaveBoundCargoPort(_ => { effects++; return PortResult.Applied; });
        ...
        {
            if (attempt > 1) foreach (ParcelId parcel in parcels) Assert.True(runtime.RetryDelivery(parcel));
            int ticks = 0;
            ...
            Assert.Equal(count * attempt, target.ReadResources().Receipts.Used);
            Assert.All(parcels, id =>
            {
                Parcel parcel = runtime.GetParcel(id);
                Assert.Equal(ParcelState.DeliveryRejected, parcel.State);
        ...
        Assert.Equal(count, source.ReadResources().Receipts.Used);
        Assert.Equal(new FlowResourceUsage(count * 16, 4096), target.ReadResources().Receipts);
        ...
        Assert.Equal(PortResult.Rejected, target.Apply(firstDelivery!)); // known receipt is readable even at 4096/4096
        Assert.Equal(count * 17, effects);
        ...
        restored.AttachSavePorts(owner, station => new SaveBoundCargoPort(_ => throw new InvalidOperationException("Restore must not apply."),
