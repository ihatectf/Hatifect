...
internal sealed class FlowPlayerWindowSequence : IDisposable
{
    ...
    internal FlowPlayerWindowSequence(IModHelper helper, string artifact,
        Func<FlowGameSession, IUiSemanticHostApi, NetworkExperience> open, Action close)
    {
        ...
        _observation = new(_api.Observation, artifact, FlowChestRoundtripAcceptance.PlayerScenario, _api.RevealAutomation);
    }
    ...
    internal bool Admit(FlowGameSession session, Chest source, Chest destination, int sx, int sy, int dx, int dy)
    {
        ...
        switch (_stage)
        {
            ...
            case 2:
                if (!Observe("player-source-registered", "Command completed")) return false;
            ...
            case 4:
                if (!Observe("player-destination-registered", "Command completed")) return false;
            ...
            case 6:
                if (!Observe("player-route-created", "Command completed")) return false;
                Activate("send");
            ...
            case 7:
                if (!Observe("player-whole-created", "Shipment created")) return false;
                Select<FlowInventorySlot>("source-cargo", value => value.Index == 1);
                Form("shipment-quantity", 0, "5"); _stage++; break;
            case 8:
                if (!Observe("player-partial-ready", "Shipment created")) return false;
            ...
            case 9:
                if (!Observe("player-partial-created", "Shipment created")) return false;
    ...
    internal bool ObserveDelivered(FlowGameSession session)
    {
        ...
        string expected = PartialParcel + " · Delivered · attempts: 1";
    ...
    private bool Observe(string name, string result, string element = "result")
    {
        ...
        return _observation.Observe(new FlowUiAcceptanceObservation.Frame(name, experience.Experience, _surface!, "en", 1, false,
    ...
    private void BeginInputProfile()
    {
    ...
    private bool CompleteInputRestore()
    {
        ...
        _restoringInput = false; _inputOptions = null;
        return true;
    }
